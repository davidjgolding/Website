          ...
          { project.github ?
              <div className="ml-auto " style={{margin:0}}>
                <Card.Link href={project.github} style={{padding: "1px 0px"}}>
                  <img width="30px" src="GitHub-Mark-64px.png" alt="GitHub Logo"/>
                </Card.Link>
              </div> 
            : null }
        </div>
...
function ControlledCarousel(state) {
  const contents = split(state.project, state.max_cards).map((projects, i) => 
    <Carousel.Item>
      <ProjectSlide projects={projects} key={i} style={{height: "90vh"}}/>
    </Carousel.Item>
    )
  return (
    <Carousel id="carousel" className="carousel-itemz" style={{height: "90vh"}} interval={null} controls={true} wrap={false} touch={false} defaultActiveIndex={state.slide}>
      {contents}
    </Carousel>
  );
}
export default class PortfolioSlide extends Component {
  ...
  updateDimensions() {
    ...
    if ($('div.active').index() > this.state.projects.length/dim) {
      this.setState({count: this.state.count+1, slide: Math.floor((this.state.projects.length)/dim)})
    }
  ...
  render() {
    ...
    return(
      <div>
        <ControlledCarousel className="jumbotron vertical-center" key={this.state.count} style={{minHeight: "90vh"}} project={this.state.projects} max_cards={this.state.max_cards} slide={this.state.slide}/>
      </div>
    );
  }
}
